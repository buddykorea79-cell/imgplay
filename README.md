...
restart 정책은 일부러 두지 않았습니다. 내 사진을 다루는 개인용 도구가 컴퓨터를
켤 때마다 백그라운드로 살아나지 않도록, 쓸 때만 띄우는 방식입니다.

### 첫 실행 순서

1. **사진 20~30장을 드롭합니다.** 폴더째 끌어다 놓아도 됩니다. 한글 파일명과
   폴더명 모두 그대로 동작합니다. 원본은 수정하지 않고 복사만 합니다.

2. **클램프 히트율부터 봅니다.** 오른쪽 위 "규칙 적합도"입니다. 개별 사진을
   예쁘게 만드는 게 아니라, *규칙이 감당하지 못하는 사진을 찾는 것*이 이 도구의
   목적입니다. 판정 기준은 아래 "클램프 히트가 이 도구의 실질적 출력입니다"에
   있습니다.

3. **클램프 배지가 붙은 사진을 눌러 봅니다.** 필름스트립에서 주황 점이 찍힌
   것들입니다. 비교 뷰에서 원본과 보정본을 좌우로 밀어 확인하고, **100% 크롭**
   모드로 바꿔 언샤프가 과한지 봅니다. 전체 보기로는 샤픈 과다가 안 보입니다.

4. **왼쪽 슬라이더로 규칙을 조정합니다.** 재측정이 없어 클램프 카운터가 즉시
   반응합니다. 목표는 특정 사진이 아니라 전체 히트율을 낮추는 것입니다.

5. **[그레이딩 확정]** 을 누릅니다. 원본 해상도 보정본이 PNG로 구워지고 2단계가
   열립니다. 사진 수에 따라 수십 초 걸립니다.

6. **2단계에서 클립 하나를 눌러 미리보기를 봅니다.** 줌·팬이 어지럽다면
   `zoom.amount`보다 `duration.base_sec`을 먼저 늘려 보세요. 같은 줌량이라도
   시간이 길면 훨씬 편안합니다.

7. **[내보내기]** 로 `grade.yaml` + `motion.yaml` + `build.sh` + `stats.csv`를
   받습니다. 이게 최종 산출물입니다 — 영상 파일이 아니라 검증된 규칙입니다.

Docker 없이 돌리려면 아래 "로컬 개발"을 보세요. ffmpeg만 직접 설치하면 됩니다.
